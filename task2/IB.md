# Какие разделы системы включить в проверочный лист?

|Раздел системы | вопросы | сервисы / бд |
|---|---|---|
|Система доступа в приложение | почему пользователи видят чужие аккаунты при входе? используется ли MFA? Ведется ли аудит доступа? | client-crm-app, crm (tenant), auth-service-1 |
| Соответстие 152-ФЗ | Проводится ли классификация данных на персональные и публичные? Обеспечено ли шифрование пдн в базах? Осуществляется ли шифроваение данных при передачи их в cdc? Используется ли обезличивание данных при передаче их в DWH? | crm-db, client-mart-db, tenant-core-db, crm-tenant-db, dwh db, очереди сообщений для сdc |
|Аудит безопасности сервисов | есть ли защита от DDos? Есть ли IDS/IPS системы? Используется ли tls при межсирвисных взаимодействиях? Закрыты ли все неиспользуемые порты на сервисах? | accountant-service-1, Витрина продаж, Витрина сервисов для собственников |
